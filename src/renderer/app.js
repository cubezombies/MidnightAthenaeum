...
const el = {
  ...
  seekWrap: $('seekWrap'), seekWaveform: $('seekWaveform'), seekPreview: $('seekPreview'),
...
const state = {
  ...
  // bookId -> Float32Array of 0-1 bar heights (see waveformHeights), or null
  // while a fetch is in flight. Populated lazily as books are opened -- see
  // loadSeekWaveform().
...
function openBook(bookId) {
  ...
  // Waveform for a richer seek bar (see waveform.js) — generated lazily on
  // first open, cached after that. A no-op if it's already cached; if not,
  // it's queued in the background and reaches patchBooks (via the same
  // library:booksUpdated push detail-fill uses) once ready, no matter which
  // book is open by the time it finishes.
  if (!book.waveformUrl) window.api.ensureWaveform(bookId);
}
...
function loadIntoPlayer(book) {
  ...
  loadSeekWaveform(book);
  renderSeekWaveform();
}

/**
 * A coarse per-book loudness waveform, drawn on a canvas layered behind the
 * plain <input type="range"> seek bar (see .seek-waveform in styles.css) --
 * the range input itself is untouched, so its native keyboard/drag/
 * accessibility behavior is exactly what it always was; only its own track
 * paint is emptied out so the canvas shows through. Generated by the main
 * process (waveform.js) lazily the first time a book is opened and cached to
 * disk; here it's just fetched once per book (over the same ab-media://
 * protocol covers already use), turned into bar heights, and kept in memory.
 */
function loadSeekWaveform(book) {
  if (!book?.waveformUrl || state.waveformCache.has(book.id)) return;
  state.waveformCache.set(book.id, null); // marks "fetch in flight" so a second call doesn't refetch
  fetch(book.waveformUrl)
    .then((res) => res.arrayBuffer())
    .then((buf) => {
      state.waveformCache.set(book.id, waveformHeights(new Uint8Array(buf)));
      if (state.playing?.id === book.id) renderSeekWaveform();
    })
    .catch(() => { state.waveformCache.delete(book.id); }); // allow a retry on next open
}

/**
 * Loudness bytes -> 0-1 bar heights, stretched to this book's own range. A
 * mastered audiobook sits in a narrow loudness band (most of it within a few
 * dB), so drawing the stored values on an absolute scale shows a nearly flat
 * bar; stretching between its 2nd and 98th percentiles makes the quiet
 * passages, pauses and loud scenes actually visible. The percentiles (rather
 * than min/max) keep one stray silent or clipped window from flattening the
 * rest. A book with essentially no variation falls back to the absolute
 * scale instead of amplifying noise into fake structure.
 */
function waveformHeights(levels) {
  ...
  for (let i = 0; i < n; i++) {
    const t = hi - lo >= 8
      ? Math.min(1, Math.max(0, (levels[i] - lo) / (hi - lo)))
      : levels[i] / 255;
    heights[i] = MIN_H + (1 - MIN_H) * t;
  }
  return heights;
}

/**
 * Redraws the seek bar's waveform canvas: bar height is relative loudness,
 * colored by played/unplayed relative to the current position (or, while
 * actively dragging, the drag target -- an instant preview of where the
 * "played" boundary would land, without waiting for the real seek), plus
 * thin chapter-boundary ticks along the top. Falls back to a plain thin
 * baseline -- the look the native track had before this feature -- when the
 * waveform hasn't loaded yet, isn't available for this book, or nothing is
 * playing.
 */
function renderSeekWaveform() {
  const canvas = el.seekWaveform;
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  ...
  const book = state.playing;
  const heights = book ? state.waveformCache.get(book.id) : null;
  const styles = getComputedStyle(document.documentElement);
  // --muted at partial opacity, not --line: --line is a divider colour, barely
  // distinguishable from the light theme's background, and the unplayed
  // portion is usually most of the bar.
  const unplayedColor = styles.getPropertyValue('--muted').trim() || '#888';
  const unplayedAlpha = 0.45;
  const playedColor = styles.getPropertyValue('--accent').trim() || '#7c6cf5';
  const tickColor = styles.getPropertyValue('--muted').trim() || '#999';

  const total = book ? effectiveDuration(book) : 0;
  const progressFrac = total > 0
    ? (state.seeking ? Number(el.seek.value) / 1000 : Math.min(1, Math.max(0, globalTime() / total)))
    : 0;

  const tickZone = Math.round(4 * dpr);
  const barTop = tickZone + Math.round(2 * dpr);
  const barAreaH = Math.max(1, h - barTop);
  const midY = barTop + barAreaH / 2;

  if (!heights) {
    ctx.globalAlpha = unplayedAlpha;
    ctx.fillStyle = unplayedColor;
    ctx.fillRect(0, midY - dpr, w, dpr * 2);
  } else {
    const n = heights.length;
    const barW = w / n;
    const gap = barW > dpr * 3 ? dpr : 0;
    for (let i = 0; i < n; i++) {
      const barH = Math.max(dpr * 2, heights[i] * barAreaH);
      const played = (i / n) <= progressFrac;
      ctx.globalAlpha = played ? 1 : unplayedAlpha;
      ctx.fillStyle = played ? playedColor : unplayedColor;
      ctx.fillRect(i * barW, midY - barH / 2, Math.max(1, barW - gap), barH);
    }
  }
  ctx.globalAlpha = 1;

  if (book?.chapters?.length > 1 && total > 0) {
    ctx.fillStyle = tickColor;
    for (const ch of book.chapters) {
      if (ch.start <= 0.5) continue; // the very start isn't a meaningful boundary
      const x = Math.min(w - dpr, (ch.start / total) * w);
      ctx.fillRect(x, 0, Math.max(1, dpr), tickZone);
    }
  }
}

/** Positions and shows the time tooltip above the seek bar at a given 0-1 fraction of the book. */
function showSeekPreview(fraction) {
  const total = effectiveDuration(state.playing);
  if (!total) return;
  const rect = el.seekWrap.getBoundingClientRect();
  if (rect.width <= 0) return;
  const clamped = Math.max(0, Math.min(1, fraction));
  el.seekPreview.textContent = formatTime(clamped * total);
  el.seekPreview.style.left = `${clamped * rect.width}px`;
  el.seekPreview.classList.remove('hidden');
}

function hideSeekPreview() {
  el.seekPreview.classList.add('hidden');
}

// Canvas backing size must track its rendered CSS size to stay crisp; a
// ResizeObserver catches the window resizing, but also the footer itself
// changing width for reasons that aren't a window resize (e.g. a scrollbar
// appearing/disappearing in the view behind it).
new ResizeObserver(() => renderSeekWaveform()).observe(el.seekWrap);
...
function updateTimeUI() {
  ...
  if (!state.seeking) renderSeekWaveform();
...
el.seek.addEventListener('input', () => {
  state.seeking = true;
  showSeekPreview(Number(el.seek.value) / 1000);
  renderSeekWaveform();
});
...
// Hover preview (before/without dragging) — mousemove over the wrap covers
// both the canvas and the real input, since the canvas has pointer-events:
// none. Dragging already gets its own live preview from the 'input' handler
// above, which also fires for keyboard-driven dragging (arrow keys) that
// this pointermove handler would never see.
el.seekWrap.addEventListener('pointermove', (e) => {
  if (state.seeking) return;
  const rect = el.seekWrap.getBoundingClientRect();
  if (rect.width <= 0) return;
  showSeekPreview((e.clientX - rect.left) / rect.width);
});
el.seekWrap.addEventListener('pointerleave', () => {
  if (!state.seeking) hideSeekPreview();
});
...
function patchBooks(updated) {
  ...
  // Separate from state.current above — the book showing in the book view
  // and the book loaded in the player aren't always the same one. Picks up
  // a freshly-generated waveform (see openBook's waveform:ensure call)
  // whichever book is currently playing.
  if (state.playing && byId.has(state.playing.id)) {
    state.playing = byId.get(state.playing.id);
    loadSeekWaveform(state.playing);
    renderSeekWaveform();
  }
